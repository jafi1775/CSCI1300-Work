...
void Map::resetMap()
{
    ...
    {
        ghost_positions_[i][0] = -1;
        ghost_positions_[i][1] = -1;
    }
    ghost_count_ = 0;

    for (int i = 0; i < max_items; i++)
    {
        item_positions_[i][0] = -1;
        item_positions_[i][1] = -1;
    }
    item_count_ = 0;
    ...
    for (int i = 0; i < num_rows_; i++)
    {
        ...
        {
            map_data_[i][j] = NOTHING;
        }
    }
    map_data_[house_escape[0]][house_escape[1]] = ESCAPE;
}
...
int Map::getHouseEscapeRow()
{
    return house_escape[0];
}

// return dungeon exit col
int Map::getHouseEscapeCol()
{
    return house_escape[1];
}
...
void Map::setHouseEscape(int row, int col)
{
    if (isOnMap(row, col))
    {
        house_escape[0] = row;
 ...
 * Return: boolean (bool)
 */
bool Map::isGhostLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    }
    for (int i = 0; i < ghost_count_; i++)
    {
        if (row == ghost_positions_[i][0] && col == ghost_positions_[i][1])
        {
            return true;
        }
    }
 ...
 * Return: boolean (bool)
 */
bool Map::isItemLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    ...
    {
        if (row == item_positions_[i][0] && col == item_positions_[i][1])
        {
            return true;
        }
    }
 ...
 *
 */
bool Map::isHouseEscape(int row, int col)
{
    if (row == house_escape[0] && col == house_escape[1])
    {
        return true;
    }
    return false;
...
bool Map::isFreeSpace(int row, int col)
{
    ...
    if (isGhostLocation(row, col))
    {
        return false;
    }
    if (isItemLocation(row, col))
    {
        return false;
    }
    if (isHouseEscape(row, col))
    {
        return false;
    }
    return true;
 ...
 * Return: boolean (bool)
 */
bool Map::addGhost(int row, int col)
{
    if (ghost_count_ >= max_ghosts)
    {
        return false;
    }
    ...
    ghost_positions_[ghost_count_][0] = row;
    ghost_positions_[ghost_count_][1] = col;
    ghost_positions_[ghost_count_][2] = false;
    map_data_[row][col] = GHOST;
    ghost_count_++;
    return true;
}
 ...
 * Return: boolean (bool)
 */
bool Map::addItem(int row, int col)
{
    ...
    item_positions_[item_count_][0] = row;
    item_positions_[item_count_][1] = col;
    item_count_++;
    map_data_[row][col] = ITEM;
    return true;
}
 ...
 * Return: boolean (bool)
 */
bool Map::removeGhost(int row, int col)
{
    for (int i = 0; i < ghost_count_; i++)
    {
        if (ghost_positions_[i][0] == row && ghost_positions_[i][1] == col)
        {
            // swap i'th npc with last npc
            ghost_positions_[i][0] = ghost_positions_[ghost_count_ - 1][0];
            ghost_positions_[i][1] = ghost_positions_[ghost_count_ - 1][1];
            ghost_positions_[i][2] = ghost_positions_[ghost_count_ - 1][2];
            // reset last npc
            ghost_positions_[ghost_count_ - 1][0] = -1;
            ghost_positions_[ghost_count_ - 1][1] = -1;
            ghost_positions_[ghost_count_ - 1][2] = false;
            // decrement npc_count_
            ghost_count_--;
 ...
 * Return: boolean (bool)
 */
bool Map::removeItem(int row, int col)
{
    for (int i = 0; i < item_count_; i++)
    {
        if (item_positions_[i][0] == row && item_positions_[i][1] == col)
        {
            // swap i'th room with last room
            item_positions_[i][0] = item_positions_[item_count_ - 1][0];
            item_positions_[i][1] = item_positions_[item_count_ - 1][1];
            // reset last room
            item_positions_[item_count_ - 1][0] = -1;
            item_positions_[item_count_ - 1][1] = -1;
            // decrement room_count_
            item_count_--;
            // set map data to explored
            //map_data_[row][col] = EXPLORED;
...
bool Map::move(char direction)
{
    ...
    if (isGhostLocation(player_position_[0], player_position_[1]))
    {
    ...
    for (int i = 0; i < num_rows_; i++)
    {
        for (int j = 0; j < num_cols_; j++)
        {
            ...
            { // NPC location, have to check if they were found yet
                for (int k = 0; k < ghost_count_; k++)
                {
                    if (ghost_positions_[k][0] == i && ghost_positions_[k][1] == j)
                    {
                        if (ghost_positions_[k][2] == true)
                        {
                            cout << GHOST;
                        }
                        else
